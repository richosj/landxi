...
function headerModal(el,target){
  var $this = $(el);
  var $target = $(target);
  ...
  if($this.is('.active')){
    ...
    $('.member-alarm,.member-info').removeClass('active')
    
    $this.addClass('active')
    $target.show().css('right' , $left);
  }
...
jQuery(function(){
  ...
  steimMap();
  // 메뉴
  $('.lay-nav-exp-btn').on('click' , function(){
    var nav = $('.lay-nav');
    if(nav.is('.wide')){
  ...
  $('.btn-show-aside').on('click' , function(){
      $('.aside_modal').addClass('open');
      return false;
  })
  $('.aside_modal .close').on('click' , function(){
      $('.aside_modal').removeClass('open');
...
$(document).ready(function(){
  ...
  $(".scrollButton").click(function(){
      ...
      // 현재 클릭한 버튼에만 활성화 클래스 추가
      //$(this).addClass("active");

      // 해당 버튼의 data-target 속성 값 가져오기
      //var target = $(this).data('target');

      // 해당 위치로 스크롤
      //$('.lay-content').animate({ scrollTop: $(target).offset().top }, 1000);

      var idx = $(this).index();

      console.log(idx)

      $('.lay-content').stop().animate({
        scrollTop : Number($('.scrollTarget').eq(idx).attr('data-scrollY'))
      },500)
  ...
  $('.scrollTarget').each(function(){
        $(this).attr('data-scrollY', $(this).offset().top - 59)
  })

  // 스크롤 이벤트 핸들러
  $('.lay-content').scroll(function(){
      // 현재 스크롤 위치
      var scrollPos = $(this).scrollTop();

      $('.scrollTarget').each(function(indx){

        console.log('스크롤 위치 : ' + scrollPos);
        if($(this).attr('data-scrollY') <= scrollPos){
          $('.info-con-sticky ul li').removeClass('active')
          $('.info-con-sticky ul li').eq(indx).addClass('active')
        }
      })
